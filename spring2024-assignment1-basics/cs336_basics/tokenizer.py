...
"""
** This is not used for training on the text, it takes the trained vocab and merges 

## Tokenizer class: 
    has vocab -> int to byte, byte to int lists
    has merges -> which will be used to encode the text
    has special_tokens -> ex. <|endoftext|>

    methods:
        from_files -> take vocab and merges from file
        vocab_size -> return vocab size
        encode -> encode the text given using the merges
        decode -> decode the ids into text
        encode_chunks -> small part of whole text encoding
        encode_iterable -> encode using generator
"""
...
from cs336_basics.utils.io_fun import get_tokenizer_from_vocab_merges_path, GPT2_PRETOKENIZER_PATTERN
